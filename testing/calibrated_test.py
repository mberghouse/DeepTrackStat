...
for f_idx in range(26,len(directories)):
    ...
    with torch.no_grad():
        for x in test_dataloader:
            ...
            if class_num==4:
                out = 1.11*(out25*40+out27*2+out19*6+out6*8+out8*4+out11)/62-.1
                out[:,0:10] = out[:,0:10]*1.1
                out[:,-5:] = out[:,-5:]*1.9
            elif class_num==3:
                out = 1.38*(out25*20+out27*1+out19*4+out6*6+out8*11+out11*8)/51-1.5
                out[:,0:10] = out[:,0:10]*1.2
                out[:,-5:] = out[:,-5:]*1.9
            elif class_num==2:
                out = 1.80*(out25*20+out27*1+out19*3+out6*6+out8*8+out11*5)/44-2
                out[:,0:10] = out[:,0:10]*1.3
                out[:,250:300] = out[:,250:300]*1.1
                out[:,-5:] = out[:,-5:]*2.0
            elif class_num==1:
                out = 1.25*(out25*35+out27*1+out19*3+out6*4+out11*2)/46-2.4
                out[:,0:10] = out[:,0:10]*1.3
