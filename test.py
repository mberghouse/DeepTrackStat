...
for f_idx in range(len(directories)):
    ...
    # with torch.no_grad():
        # for x in test_dataloader2:
            ...
            # out0 =F.softmax((model4((x).to(device)).detach().cpu()),1).numpy()
            # out1 =F.softmax((model5((x).to(device)).detach().cpu()),1).numpy()
            # out2 =F.softmax((model6((x).to(device)).detach().cpu()),1).numpy()
            # out3 =F.softmax((model4((x).to(device)).detach().cpu()),1).numpy()
