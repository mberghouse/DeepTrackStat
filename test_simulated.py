...
for f_idx in range(0,len(directories)):
    ...
    plt.title('Turn Angle Vectors for '+str(base)+', Class Number = '+str(class_num_ang) , fontsize=10)
    plt.legend(['Predictions', 'Particle Tracking (TrackMate)', 'Ground Truth'])
    #plt.xscale('log')
    #plt.yscale('log')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('Results/Turn_angle_'+str(f_idx)+'.png')
    w1 = wasserstein_distance(distr_torch1, distr_torch)
    w2 = wasserstein_distance(distr_torch2, distr_torch)
    print ('Earth Movers Distance (model, PT): ', w1, w2)
    ang_w1.append(w1)
    ...
    with torch.no_grad():
        for x in test_dataloader2:
            ...
            if class_num == 4:
                out = .85*(out0*.45+out1*2.9+2.1*out2)/5
            elif class_num == 3:
                out = .9*(out0*.55+out1*2+1.7*out2+out3*2)/5
            elif class_num == 2:
                out = 1.02*(out1*1.8+1.9*out3+out2*1.6)/4
            elif class_num == 1:
               out = 1.17*(out1*3+1.6*out2)/4
            else:
                out = 1.25*(out3*2+out2*2+out1*7+out0*.8)/11
    ...
    print('True Directionality: ', direction)
...
plt.figure(figsize=(8, 6)) 
plt.scatter( final_out_direction, final_true_direction, s=20, c='g')
plt.title('Directionality Predictions vs Ground Truth')
plt.xlim([0,1])
plt.ylim([0,1])
